...
const useCode = () => {
    ...
    const cstmrurl = "http://127.0.0.1:8000/api/customers";
...
const handleSignup = (e) => {
  ...
  if (Object.keys(validationErrors).length === 0) {
    ...
    axios({
      method: "POST",
      url: "http://127.0.0.1:8000/api/customers/send",
      data: getData,
      headers: { 'Content-Type': 'multipart/form-data' }
    })
    .then((response) => {
      return axios.get(cstmrurl);
    })
    .then((response) => {
...
const pc = (item) => {
  return item.prod_category === "PC";
};

const phone = (item) => {
  return item.prod_category === "Smart Phone";
};
  ...
  axios.get(cstmrurl)
    .then((response) => {
      ...
      if (Array.isArray(usersArray)) {
        const isUserExist = usersArray.some((user) => 
          user.email === userLogin.uname && user.password === userLogin.upass
        );
        ...
        console.error("Expected an array but got:", usersArray);
      }
    })
};
    ...
    const addToCart = (item) => {
      ...
      axios({
        method: "POST",
        url: "http://127.0.0.1:8000/api/cart/send",
        data: getData,
        headers: { 'Content-Type': 'multipart/form-data' }
      })
        .then(() => {
    ...
    return {
        ...
        userLogin, setUserLogin, navigate, arr, setArr, url, handleSignup, handleLogin,
        ...
        addToCart, cartCount, setCartCount, setGamingPhone,
        ...
        showSuccess, setShowSuccess, paymentMethod,  pcImages, gPhoneImages, 
